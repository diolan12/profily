<div id="payment" class="scrollspy">

    <div class="container">
        <div class="section">

            <div class="row">
                <div class="col s12 center" data-aos="fade-up">
                    <h4 class="font-recursive">Payment Terms</h4>
                    <div class="col s12 m6" data-aos="fade-up">
                        <h5 class="font-recursive">We Do</h5>
                        <p><i class="material-icons left green-text">done</i>Accept T/T in advance or 100% payment</p>
                        <p><i class="material-icons left green-text">done</i>100% L/C irrevocable at sight</p>
                    </div>
                    <div class="col s12 m6 center" data-aos="fade-up">
                        <h5 class="font-recursive">We Do Not</h5>
                        <p><i class="material-icons left red-text">close</i>We do not accept DDP, USANCE LC, REVOCALE LC</p>
                    </div>
                </div>
            </div>

        </div>
    </div>
</div>